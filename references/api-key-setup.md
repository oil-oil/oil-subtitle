...
| 配置名 | 业务环境变量 | 系统凭据引用 |
| --- | --- | --- |
| default | `DASHSCOPE_API_KEY` | `oil-subtitle/dashscope/default` |
...
```bash
node "$SKILL_DIR/scripts/credential-ui/src/profile.ts" run default -- "$SKILL_DIR/.venv/bin/python3" "$SKILL_DIR/scripts/bailian_transcribe.py" "/path/to/video.mp4" --output "/path/to/transcript.json"
```
